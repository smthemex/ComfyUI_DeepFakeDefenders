...
class MFF_MoE(nn.Module):
    ...
    def load(self, path):
        ...
        path_a=os.path.join(path,'weight.pth')
        path_b = os.path.join(path, 'ema.state')
        weights_file = torch.load(path_a, map_location="cpu")
        self.experts.load_state_dict(weights_file)

        # load ema weights
        self.ema_state = torch.load(path_b,map_location="cpu")
